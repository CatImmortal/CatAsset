...
namespace CatAsset.Editor
{
    ...
    /// 资源信息树视图
    /// </summary>
    public class AssetInfoTreeView : BaseProfilerTreeView
    {
        /// <summary>
        /// 是否只显示主动加载的资源
        /// </summary>
        ...
        {
            return ProfilerInfo != null && ProfilerInfo.AssetInfoList.Count > 0;
        }
        ...
        public override void OnSortingChanged(MultiColumnHeader header)
        {
            ...
            switch (column)
            {
                ...
                case ColumnType.Object:
                    assetOrdered = ProfilerInfo.AssetInfoList.Order(info => info.Name, ascending);
                    break;

                case ColumnType.Type:
                    assetOrdered = ProfilerInfo.AssetInfoList.Order(info => info.Type, ascending);
                    break;

                case ColumnType.Group:
                    assetOrdered = ProfilerInfo.AssetInfoList.Order(info => info.Group, ascending);
                    break;

                case ColumnType.Bundle:
                    assetOrdered = ProfilerInfo.AssetInfoList.Order(info => info.Bundle, ascending);
                    break;

                case ColumnType.Length:
                    assetOrdered = ProfilerInfo.AssetInfoList.Order(info => info.Length, ascending);
                    break;

                case ColumnType.RefCount:
                    assetOrdered = ProfilerInfo.AssetInfoList.Order(info => info.RefCount, ascending);
                    break;

                case ColumnType.UpStreamCount:
                    assetOrdered = ProfilerInfo.AssetInfoList.Order(info => info.DependencyChain.UpStream.Count, ascending);
                    break;

                case ColumnType.DownStreamCount:
                    assetOrdered = ProfilerInfo.AssetInfoList.Order(info => info.DependencyChain.DownStream.Count, ascending);
                    break;
            ...
            {
                ProfilerInfo.AssetInfoList = new List<ProfilerAssetInfo>(assetOrdered);
                Reload();
            }
        }
        ...
        protected override TreeViewItem BuildRoot()
        {
            ...
            foreach (var assetInfo in ProfilerInfo.AssetInfoList)
            {
